...
<template>
  <div>
    ...
    <el-table :data="page.list" style="width: 100%" :row-class-name="tableRowColor">
      ...
      <el-table-column prop="nowMonthNums" label="当月用量(立方米)" width="280">
      </el-table-column>
      <el-table-column prop="nowPrices" label="当月气费(元)" width="280">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getPage(pageNum) {
      ...
      this.$axios.post("http://localhost:8080/gas/selectSum/" + this.monthCom, this.houseInfo
      ).then(resp => {
        if (resp.data.code === 200) {
